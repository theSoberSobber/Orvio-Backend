...
export class FcmController {
  ...
  async sendDataMessage(@Payload() data: { fcmToken: string; otp: string; phoneNumber: string; tid: string, timestamp: string }) {
    // console.log("Message recieved at the fcm service with data...", data);
    return this.fcmService.sendServiceMessage(data.fcmToken, data.otp, data.phoneNumber, data.tid, data.timestamp);
  }
